<template>
  <div>
    <div class="home bg-cover bg-center bg-no-repeat h-screen">
      ...
      <!-- <h1
        class="md:text-5xl text-xl text-gray-100 mt-32 md:ml-10 ml-20 md:w-1/2 leading-relaxed bg-black"
      >
        <span class="text-red-600 font-bold ">CinemaHall </span> THE PLACE OF
        ENTERTAINMENT
      </h1> -->
    </div>
    <h1 class="md:text-4xl text-2xl md:ml-16 ml-8 md:mt-20 mt-10 font-bold flex justify-end ...">
      <input
        class="py-1 px-6 mx-12 border-2 border-yellow-100 rounded text-sm"
        type="date"
        v-model="date"
        @change="filter()"
      />
...
export default {
  ...
  mounted() {
    axois.get("http://localhost/CinemaHall/api/movies")
    .then((res) => {
      this.movies = res.data;
    });
    // this.filter()
