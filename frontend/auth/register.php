...
<script>
        ...
        let registerSubmit = document.getElementById('registerForm');
        registerSubmit.addEventListener('submit', function(e){
        e.preventDefault();
        // var data = {text: "test from app.html"}
        const formData = new FormData(registerSubmit);
      ...
      const options ={
        ...
        .then(Request=>Request.json()).then(myData=>{
          ...
          myData.token);
          }else if(!(myData.token)){
            Swal.fire ('You already have an account with this email!')
